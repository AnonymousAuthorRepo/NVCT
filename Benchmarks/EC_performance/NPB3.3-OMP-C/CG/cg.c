...
/*
  //EC: candidates
  crucial_data(x,"double", NA+2);  //critical
  crucial_data(z,"double", NA+2);  //critical
  crucial_data(p,"double", NA+2);
  crucial_data(q,"double", NA+2);
  crucial_data(r,"double", NA+2);
...
//*/ 
  ...
  if (Class != 'U') {
    ...
    if (err <= epsilon) {
      ...
      printf(" Cache flushing overhead is %lf\n", tflush/num_threads );
    } else {
      verified = false;
...
/*
    ...
    //flush_dcache_range(&r[0], &r[NA]+sizeof(double));
